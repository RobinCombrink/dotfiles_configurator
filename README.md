...
# Enact without being asked, which a run with no terminal needs
cargo run -- apply --machine personal --yes
```
...
- **The configurator updates itself from an origin it carries** (ADR 0019). Its own currency is a resolved resource in every change set that no configuration declares — plan reports it, apply converges it. That is also what lets the generation floor be escaped: a configuration stating a generation above the build cannot be read, so an apply obtains a newer build once and reads again rather than needing a person.
...
- **The schema is generated from the types.** A test renders [`schema/configuration_schema.json`](schema/configuration_schema.json) from the same types the tool reads configurations with and fails when the committed file differs, so it cannot describe a shape the tool would refuse. `UPDATE_SCHEMA=1 cargo test --test schema` regenerates it.
